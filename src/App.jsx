      ...
      try {
        if (!auth.currentUser) {
          if (typeof __initial_auth_token !== 'undefined' && __initial_auth_token) {
            ...
            // await signInAnonymously(auth); // Nicht mehr automatisch anonym anmelden, wenn wir Google wollen
          }
        }
      } catch (e) {
        ...
        <div className="flex items-center gap-2">
          ...
          {user && !user.isAnonymous && user.photoURL ? (
            <div className="relative flex items-center">
              <button onClick={() => setShowProfileMenu(!showProfileMenu)} className="w-11 h-11 rounded-full overflow-hidden border border-slate-200 shadow-sm transition-transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-indigo-500/20">
                <img src={user.photoURL} alt="Profil" className="w-full h-full object-cover" />
              </button>
              {showProfileMenu && (
                <div className="absolute right-0 top-12 bg-white rounded-2xl shadow-xl border border-slate-100 p-4 w-64 z-50 animate-in zoom-in-95 origin-top-right">
                  <div className="flex items-center gap-3 mb-4 pb-4 border-b border-slate-50">
                    <img src={user.photoURL} className="w-12 h-12 rounded-full border border-slate-100" alt="" />
                    <div className="overflow-hidden">
                      <p className="font-black text-sm text-slate-800 truncate">{user.displayName}</p>
            ...
            <button onClick={onLoginRequest || handleGoogleLogin} className={`flex items-center gap-2 px-3 sm:px-4 py-2.5 bg-white border border-slate-200 text-slate-700 rounded-2xl font-bold transition-all hover:bg-slate-50 active:scale-95`} title="Mit Google anmelden">
